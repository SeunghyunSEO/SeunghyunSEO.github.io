...
(+ 또하나의 문제는 Optimization 하는 과정이 MSE Loss를 사용한 Supervised Regression 문제 같이 보이지만 Target Value가 계속 변화하는 문제입니다.)
...
(이 때 데이터는 딥러닝 알고리즘들이 사용하는 데이터가 `iid`라고 가정하는 것과 같다고 합니다.)  
...
이렇게 Q 함수를 근사한 네트워크를 업데이트하는 Value-based Objective와 Replay-Buffer ...
하지만 여기에 거대한 state space를 근사하는데 효과적이고 편리한 딥러닝 네트워크를 적절히 결합한 네트워크는 굉장히 놀라운 결과를 보여주는데요, 그것이 바로 아직까지도 많이 사용되는 Value-Based Deep RL 알고리즘들의 근간이 되는  `Deep-Q-Network (DQN)` 입니다.
...
`DQN`은 2013년에 제안된 딥러닝과 강화학습 처음 결합한 네트워크이며,
`Atari`라는 보드 게임에서 인간 수준의 퍼포먼스를 보여주며 심층 강화학습의 가능성을 보여준 간단하지만 강력한 네트워크입니다.

![2013_dqn](/assets/images/CS285/lec-8/2015_dqn.png)
*Fig. ScreenshotsfromfiveAtari2600Games:(Left-to-right)Pong,Breakout,SpaceInvaders, Seaquest, Beam Rider. from [2013, Playing Atari with Deep Reinforcement Learning](https://arxiv.org/pdf/1312.5602)*
