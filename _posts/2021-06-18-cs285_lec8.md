...
이렇게 Q 함수를 근사한 뉴럴 네트워크를 업데이트하는 Value-based Objective와 Replay-Buffer ...
그리고 여기에 거대한 state space를 근사하는 네트워크까지 같이 사용하면 굉장히 놀라운 결과를 보여주는데요, 
그것이 바로 아직까지도 많이 사용되는 Value-Based Deep RL 알고리즘들의 근간이 되는  `Deep-Q-Network (DQN)` 입니다.
...
여기서 잊지말아야 할 점은 앞서 말한것 처럼 리플레이 버퍼에 데이터를 추가해주는 과정이 있다는 겁니다.
...
### <mark style='background-color: #dcffe4'> Dueling-DQN (DDQN) </mark>
...
![dueling_dqn1](/assets/images/CS285/lec-8/dueling_dqn1.png)
*Fig.*
...
### <mark style='background-color: #dcffe4'> DQN vs DDQN </mark>
