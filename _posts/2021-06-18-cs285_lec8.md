...
일반적으로 뉴럴 네트워크로 근사한 함수는 최적화 하기가 쉽지 않은데요, 이보다 단순한 형태인 2차식 형태로 Q-Function을 나타내는 겁니다. 이 2차식의 optimal solution은 closed-form 형태로 구할 수 있기 때는데요, `Normalized Advantage Function (NAF)`에서는 어떤 뉴럴 네트워크가 state, $$s$$를 인풋으로 받아 $$\mu,P,V$$로 표현되는 벡터나 매트릭스의 세 가지 qunatity를 출력값으로 뱉습니다. 
