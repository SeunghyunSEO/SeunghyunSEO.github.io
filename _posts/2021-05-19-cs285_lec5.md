...
$$
\bigtriangledown_{\theta}J(\theta) \approx \sum_{i=1}^N \bigtriangledown_{\theta} log \pi_{\theta} (\tau_i) r(\tau_i)
$$

이는 N개의 Trajectory에 대해서 각 Trajectory마다 매 time-step을 고려하지 않고, 그냥 Trajectory만 고려해서 미분을 해 업데이트한다는 것이며,  
좋았던 Trajectory (Reward가 큰 양수)가 나올 확률을 높히고 아닌 Trajectory (음수)는 낮추는게 되는 것 같네요. (즉 좋은 행동을 권장하는게 아니라 좋은 Trajectory를 권장하는게 되는건데, 그게 그건가...?)
