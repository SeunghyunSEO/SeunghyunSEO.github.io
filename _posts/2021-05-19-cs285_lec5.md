...
& 
\bigtriangledown_{\theta'} J(\theta') = \mathbb{E}_{\tau \sim p_{\theta}(\tau)}[ ( \sum_{t=1}^T \bigtriangledown_{\theta'} log \pi_{\theta'}(a_t \vert s_t) ) (\prod_{t'=t}^t \frac{ \pi_{\theta'}(a_{t'} \vert s_{t'}) }{ \pi_{\theta}(a_{t'} \vert s_{t'}) })  ( \sum_{t=1}^T r(s_t,a_t) ) (\prod_{t''=t}^{t'} \frac{ \pi_{\theta'}(a_{t''} \vert s_{t''}) }{ \pi_{\theta}(a_{t''} \vert s_{t''}) })] 
& \\
\end{aligned}
$$
...
하지만 이러한 사실이 log probability에 곱해지는 $$(\prod_{t'=t}^t \frac{ \pi_{\theta'}(a_{t'} \vert s_{t'}) }{ \pi_{\theta}(a_{t'} \vert s_{t'}) }) $$에도 적용되는 사실은 아니라고 하는데요, 이것을 해결하는것도 골칫거리입니다.
...
$$(\prod_{t'=t}^t \frac{ \pi_{\theta'}(a_{t'} \vert s_{t'}) }{ \pi_{\theta}(a_{t'} \vert s_{t'}) }) $$가 문제가 되는 이유는 이 텀이 $$t$$에 지수승 (exponential)이기 때문입니다. 
이 `Importance Ratio` 값은 1보다 작은 값인데, 이 값이 수없이 곱해지다보면 0에 가깝게 수렴하게 되는데요, 이는 달리말하면 Variance가 굉장히 빠르게 무한대로 발산한다는 것을 의미한다고 합니다.
...
$$
\bigtriangledown_{\theta'}J(\theta') \approx \frac{1}{N} \sum_{i=1}^N \sum_{t=1}^T ( \prod_{t'=t}^t  \frac{ \pi_{\theta'}(a_{i,t'} \vert s_{i,t'}) }{ \pi_{\theta}(a_{i,t'} \vert s_{i,t'}) } ) \bigtriangledown_{\theta} log \pi_{\theta} (a_{i,t} \vert s_{i,t}) \hat{Q_{i,t}}
$$
