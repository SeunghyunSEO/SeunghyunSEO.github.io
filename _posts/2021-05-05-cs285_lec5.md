...
$$
p_{\theta}(\tau) \bigtriangledown_{\theta} log p_{\theta} (\tau) = \not{p_{\theta}(\tau)} \frac{ \bigtriangledown_{\theta} p_{\theta} (\tau) }{ \not{p_{\theta} (\tau)} }
$$
...
$$
\bigtriangledown_{\theta} log p_{\theta}(\tau) = \bigtriangledown_{\theta} [\not{log p(s_1)} \sum_{t=1}^T log \pi_{\theta}(a_t \vert s_t) + \not{log p(s_{t+1} \vert s_t, a_t )}]
$$
