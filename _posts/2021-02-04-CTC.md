...
*Alex Graves* 선생님께서는 이를 아래와 같이 설명하고 있습니다.

```
For a given transcription sequence, there are as many possible alignments as there different ways of separating the labels with blanks. 
For example (using ‘−’ to denote blanks) the alignments (a, −, b, c, −, −) and (−, −, a, −, b, c) both correspond to the transcription (a, b, c). 
When the same label appears on successive time-steps in an alignment, the repeats are removed: therefore (a, b, b, b, c, c) and (a, −, b, −, c, c) also correspond to (a, b, c).
```



위에서 설명한 '정답 sequence에 대해 가능한 어떤 path 중 하나($$a$$)'란 바로 이런것입니다.

아래의 수식이 의미하는 바가 어떤것인지 감이 오시나요? ```_HHHHEEEELLLO...``` 이런것에 대한 확률값을 뽑는겁니다.
...
자 이제 다했습니다, 우리는 likelihood를 정의했으며, 일반적으로 Maximum likelihood는 negative log likelihood로 만들어 이를 최소화 하는 문제라고 할 수 있으니, 
