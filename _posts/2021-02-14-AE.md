...
결론적으로 우리는 `Lower Bound`를 $$p$$와 $$q$$ 모두에 대해서 최대화 하는 것이 결국 `생성 모델이 원하는 진짜 목적 함수 (Objective Function)`, $$log p(x)$$를 최대화 하게 되는 겁니다.

- Lower Bound를 $$q$$에 대해서 최대화 : $$p,q$$ 사이의 KLD가 줄어든다 - `bound가 tight해진다`
- Lower Bound를 $$p$$에 대해서 최대화 : $$log p(x_i) \geq L_i$$의 $$L_i$$이 커지면서 $$log p(x_i)$$도 덩달아 커진다.
...
조금 더 디테일하게는 아래의 flow를 따르면 되겠습니다.
...
& \space \text{calculate } \bigtriangledown_{\theta} L_i(p,q_i): & \scriptstyle{\text{; calculate gradient of ELBO with respect to model param}} \\
& \space \space \text{sample } z \sim q_i(z) & \scriptstyle{\text{; 1. sample z because we have Expectation}}  \\
& \space \space \bigtriangledown_{\theta} L_i(p,q_i) \approx \bigtriangledown_{\theta} log p_{\theta} (x_i \vert z) &  \scriptstyle{\text{; 2. calculate gradient for one sample (can use multiple sample). p(z) has no parameter}}  \\
& \space \theta \leftarrow \theta + \alpha \bigtriangledown_{\theta} L_i(p,q_i) &  \scriptstyle{\text{; 3. update param on } \theta} \\
& \space \text{update } q_i \text{ to maximize }  L_i(p,q_i) &  \scriptstyle{\text{; 4. also maximize } q_i \text{how? -> will talk bout this later}}
