...
*Fig. Variational AutoEncoder의 모식도 2, 여기서는 우리가 추정하고자 하는 latent distribution z가 가우시안 분포라고 생각하기 때문에 mean, variance를 찾게 됩니다.*


우리는 데이터로부터 단순히 고정된 벡터 (fixed vector)가 아닌 분포 (distribution)을 찾아내고 싶다는 것을 다시 리마인드 하고, 이제 VAE가 무엇인지 한번 제대로 알아보도록 하겠습니다.
...
```
여기서 우리가 분포를 배운다는 것이 가지는 의미가 무엇인지 생각해 볼 필요가 있습니다.
직관적인 이해를 위해서 우리가 x를 입력으로 했을 때 z라는 fake label을 만든다고 생각하면 될 것 같습니다. 예를 들어 가우시안 분포를 가정하면, 
x1이라는 이미지(i.e. 개 이미지)는 5라는 값을 정답으로 가질 확률이 가장 높고, x2라는 이미지 (i.e. 고양이 이미지)는 10이라는 값을 가질 확률이 가장 높고 ...  
이런식으로 실제 데이터셋에는 존재하지 않지만우리가 임의로 해당 입력값에 대한 정답 분포가 있다고 가정하는 겁니다.
```
...
1. 우리가 학습을 통해서 
2. 우선, 사전 분포 (prior), $$p_{\theta^*}(\mathbf{z})$$로 부터 $$\mathbf{z}^{(i)}$$를 샘플링 합니다. 
3. 그리고 다음의 식, $$p_{\theta^*}(\mathbf{x} \vert \mathbf{z} = \mathbf{z}^{(i)})$$를 통해 $$z$$가 주어졌을 때 $$\mathbf{x}^{(i)}$$를 만들어 냅니다(디코딩).
