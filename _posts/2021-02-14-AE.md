...
목적 자체도 오토인코더와 같이 차원축소를 목적으로 '저차원의 유의미한 representation을 추출한다' 가 아니라, `'데이터로부터 잠재 분포(latent distribution)를 찾아내고, 이로부터 샘플링을 통해 데이터셋에 없는 그럴싸한 새로운 데이터를 만들어내는 생성 모델(generative model)을 만든다'` 입니다.

$$x$$를 넣어서 원래 입력값인 $$\mathbf{x} \approx f_\theta(g_\phi(\mathbf{x}))$$를 복원해내는 비지도 학습 방식으로 학습되는 것은 똑같습니다.

![lee_ae1](/assets/images/ae_to_vae/lee_ae1.png)
...
![lee_vae1](/assets/images/ae_to_vae/lee_vae1.png)
...
![lee_vae2](/assets/images/ae_to_vae/lee_vae2.png)
...
그리고 VAE는 AE와 다르게 입력 데이터로부터 단순히 고정된 벡터 (fixed vector)를 찾는게 아니라 분포 (distribution)를 찾아내는게 목적입니다.
이제 VAE에 대해서 차분히 알아보도록 할건데, 그 전에 우리는 `잠재 변수 모델 (Latent Variable Model)`이 무엇이며 어떤 의미를 가지는지에 대해 알 필요가 있습니다.



### <mark style='background-color: #dcffe4'> What is Latent Variable Model ? </mark>

머신러닝은 일반적으로 출력 값의 분포 (Output Distribution)를 적절하게 가정하고, 이를 나타내기 위한 파라메터를 찾는것이 목적입니다.
이 분포는 $$p(y \vert x)$$ 가 될 수도 있고, $$p(x)$$가 될 수도 있죠.

잠재 변수 모델은 우리가 가지고 있는 입력 데이터 $$x$$의 분포 $$p(x)$$를 어떤 알려지지 않은 변수 $$z$$로 `Integrated Out` 하는 것으로부터 시작됩니다. 

$$
p(x) = \int p(x \vert z) p(z) dz
$$

위의 수식에서 $$p(x \vert z)$$와 $$p(z)$$는 보통 계산상의 편의를 위해 가우시안 분포와 같은 쉬운 분포를 가정합니다.

여기서 `잠재 변수 (latent variable distribution)`, $$p(z)$$ 이 의미하는 바는 뭘까요?


![cs285_slide2](/assets/images/ae_to_vae/cs285_slide2.png)
*Fig. a*



![cs285_slide3](/assets/images/ae_to_vae/cs285_slide3.png)
*Fig. a*
...
VAE의 모식도에 따라서 설명하자면, VAE의 과정은 아래와 같습니다.
...
```
여기서 우리가 잠재 분포 (Latent Distribution)를 배운다는 것이 어떤 의미를 가지는지 생각해 볼 필요가 있습니다.

직관적인 이해를 위해서 우리가 x를 입력으로 했을 때 z라는 fake label을 만든다고 생각하면 될 것 같습니다. 
예를 들어 가우시안 분포를 가정하면, x1이라는 이미지(i.e. 개 이미지)는 5라는 값을 정답으로 가질 확률이 가장 높고, x2라는 이미지 (i.e. 고양이 이미지)는 10이라는 값을 가질 확률이 가장 높고 ...  

이런식으로 실제 데이터셋에는 존재하지 않지만 입력 데이터에 내재된 (latent) 우리가 임의로 해당 입력값에 대한 정답 분포 (distribution) 가 있다고 가정하는 겁니다.
```


그리고 우리는 아래와 같은 용어를 정의하여 앞으로 VAE 수식을 전개하는데 사용하려고 합니다.
...
여기서 우리가 학습을 통해 찾은 디코더의 optimal parameter를 $$\theta^{\ast}$$ 라고 하면 우리는 이 값을 아래의 수식을 통해 표현할 수 있습니다.
