...
![jd_gmm6](/assets/images/ae_to_vae/jd_gmm6.png){: width="70%"}
*Fig. 복잡한 분포를 가지는 $$p(x)$$*
...
![jd_gmm1](/assets/images/ae_to_vae/jd_gmm1.png){: width="70%"}
*Fig. 잠재 변수 (여기서는 $$h$$라는 노테이션으로 쓰임)와 원본 데이터 $$x$$와의 결합 분포, $$h$$의 분포는 그림에서는 연속적인 분포다 (i.e.가우시안)*

만약의 잠재 변수가 이산적인 분포를 갖는 경우를 가정하면 이는 아래와 같은데요,

![jd_gmm7](/assets/images/ae_to_vae/jd_gmm7.png){: width="70%"}
...
![jd_gmm3](/assets/images/ae_to_vae/jd_gmm3.png){: width="70%"}
![jd_gmm4](/assets/images/ae_to_vae/jd_gmm4.png){: width="70%"}
*Fig. 학습을 통해 알아서 데이터의 분포를 더욱 잘 설명하는 복잡한 분포를 찾아내는 것을 알 수 있다.*

이를 2차원이나 고차원으로 확장시켜도 문제가 없습니다.
...
우리는 이러한 잠재 변수 모델을 보통 `EM Algorithm`을 통해서 학습하곤 합니다.
...
이제 이를 더 확장해서 (generalization) 생각해보도록 하겠습니다.
