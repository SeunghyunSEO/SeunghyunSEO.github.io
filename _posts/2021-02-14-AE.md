...
{: class="info"}
| Symbol | Input | Output | parameter | 목적 |
| ---------- | ---- | ----|----------|
| $$q_{\phi}(\mathbf{z}\vert\mathbf{x})$$ | x (i.e. 이미지) | $$\mu$$,$$\sigma$$ (가우시안으로 가정했기 때문) | $$\phi$$ |  x를 입력으로 분포를 추론 (Inference) | 
| $$p(z)$$ | - | - | - | 말 그대로 사전 분포. $$q_{\phi}(z \vert x)$$는 이 분포를 닮아야 함. |
| $$p_{\theta}(\mathbf{x}\vert\mathbf{z})$$ | z ($$q_{\phi}(z \vert x)$$에서 샘플링한 벡터) | x (i.e. 생성된 이미지) | $$\theta$$ | 생성 (Generation) |
| ---------- | ---------- | ----------|----------|
