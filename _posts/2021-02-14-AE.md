...
이 수식을 해석하는 또 다른 방법은 바로 `쿨백 라이블러 발산 (KL-Divergence)`을 이용하는 건데요,
KL-Divergence는 서로 다른 두 분포에 대해서 다음의 수식을 만족합니다.
...
KL-Divergence에 대해서도 할말이 굉장히 많지만, 정말 간단하게 얘기해서 KL-Divergence는 `'두 분포가 얼마나 다른가'` 혹은 `'두 분포간의 거리'`를 의미하며, 수식의 결과값이 작을수록 두 분포가 유사합니다. 
