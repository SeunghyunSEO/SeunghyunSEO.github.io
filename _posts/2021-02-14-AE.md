...
그렇기 때문에, 우리는 `인코더를 학습 시키기 위해서` 입력 데이터 x를 네트워크에 통과시켜 다시 x를 만들어내게끔 하는 `비지도 학습 (Unsupervised Learning)`방법으로 네트워크를 학습하기 위해서 다시 저차원의 압축된 정보를 원래의 차원으로 복원시켜줄 디코더(Decoder)를 덧붙혀서 학습을 시키게 됩니다.
...
오토인코더는 위와같이 네트워크가 좁아졌다가 다시 넓어지는 형태로 생겼습니다. 가운데 차원이 확 좁아지는 곳은 물병의 목 같다고 해서 `Bottle Neck` 이라고 합니다. 
...
![jd_prob1](/assets/images/ae_to_vae/jd_prob1.png){: width="50%"}
...
여기서 아래와 같은 특성을 이용하면, (Janson's Inequality)
...
생성 모델 이란 결국 $$log p(x) = \sum_i log p(x_i) $$를 최대화 하는 문제인데, $$\mathbb{E}_{z \sim q_i(z)} [log p(x_i \vert z) + log p(z)] - \mathbb{E}_{z \sim q_i(z)} [log q_i(z)]$$ 라는 term이 lower bound이기 때문에 이를 최대화 하는 것이 곧 $$log p(x)$$ 를 최대화 하는 문제를 푸는 것과 다름 없게 .
...
- Lecture Videos
  ...
  - 4. [L4 Latent Variable Models (VAE) -- CS294-158-SP20 Deep Unsupervised Learning -- UC Berkeley from Pieter Abbeel](https://www.youtube.com/watch?v=FMuvUZXMzKM)
  - 5. ['오토 인코더의 모든 것 (1~3)' from Hwalseok Lee](https://www.youtube.com/watch?v=o_peo6U7IRM) 
- Other 
