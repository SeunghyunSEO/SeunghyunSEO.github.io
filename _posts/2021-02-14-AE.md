---
title: (미완) A (Long) Peek into Deep Generative Models - From AutoEncoder (AE) to Variational AutoEncoders (VAEs)
...
하지만 이번 글에서는 AE에 대해서만 알아볼 것이고, 더 나아가 다양한 목적을 위한 AE의 Variation들 (DAE 등) 그리고 VAE와 다양한 VAE의 Variation들 (VQ-VAE 등) 에 대해서도 알아보도록 하겠습니다.
...
*Fig. AE가 원하는 것은 입력 데이터보다 상대적으로 저 차원인 잠재 벡터 (hidden vector)를 출력해주는 Encoder를 학습하는 것이다.*
...
$$h=1,2,\cdots$$ 같은 `가짜 정답 (Fake Label)` 을 만들고 알아서 학습하게 되는거죠.
...
여기서 적분식이 가지는 의미는 우리가 연속적인 분포 $$p(z)$$의 가능한 모든 변수들을 고려하겠 다는 것인데, 이는 불가능 하기 때문에 적분식을 계산하는 것은 intractable합니다. (즉 no closed-form solution). 
...
여기서 과연 $$p(z \vert x_i)$$을 어떻게 계산할 것인가? 가 관건인데,
이를 해결하기 위한 `probabilistic inference`중 가장 유명하고, 자주 쓰이는 방법이 바로 `변분 추론 (variational Inference)`이 됩니다.
...
$$
\begin{align}
log p(x_i) = \mathbb{E}_{z \sim q_i(z)} [ log \frac{p(x_i \vert z) p(z) }{q_i(z)} ] = \mathbb{E}_{z \sim q_i(z)} [log p(x_i \vert z) + log p(z)] - \mathbb{E}_{z \sim q_i(z)} [log q_i(z)] \\
log p(x_i) = \mathbb{E}_{z \sim q_i(z)} [ log \frac{p(x_i \vert z) p(z) }{q_i(z)} ] = \mathbb{E}_{z \sim q_i(z)} [log p(x_i \vert z) + log p(z)] - H(q_i)
\end{align}
$$


$$
D_{KL} (q \parallel p) = \mathbb{E}_{x \sim q(x)} [log \frac{q(x)}{p(x)}] = \mathbb{E}_{x \sim q(x)} [log q(x)] - \mathbb{E}_{x \sim q(x)} [log p(x)] = - \mathbb{E}_{x \sim q(x)} [log p(x)] - H(q)
$$
...
결국 $$L_i (p, q_i)$$를 $$q_i$$ 에 대해서  최대화 하는 것이 KL-divergence 를 최소화 하는 것이 됩니다.
...
for \space each \space x_i (or \space mini-batch): \\
\space calculate \space \bigtriangledown_{\theta} L_i(p,q_i): \\
\space \space sample \space z \sim q_i(z) \\
\space \space \bigtriangledown_{\theta} L_i(p,q_i) \approx \bigtriangledown_{\theta} log p_{\theta} (x_i \vert z) \\
\space \theta \leftarrow \theta + \alpha \bigtriangledown_{\theta} L_i(p,q_i) \\
\space update \space q_i \space to \space maximize \space L_i(p,q_i)

\end{align}
$$


- if $$ q_i(z) = N(\mu_i, \sigma_i) $$
...
$$
log p(x_i) \geq \mathbb{E}_{z \sim q_{\phi}(z \vert x_i)} [log p(x_i \vert z) + log p(z)] + H(q_{\phi}(z \vert x_i))
$$
...
for \space each \space x_i (or \space mini-batch): \\
\space calculate \space \bigtriangledown_{\theta} L(p_{\theta}(x_i \vert z), q_{\phi}(z \vert x_i)): \\
\space \space sample \space z \sim q_{\theta}(z \vert x_i) \\
\space \space \bigtriangledown_{\theta} L \approx \bigtriangledown_{\theta} log p_{\theta} (x_i \vert z) \\
\space \theta \leftarrow \theta + \alpha \bigtriangledown_{\theta} L \\
\space \phi \leftarrow \phi + \alpha \bigtriangledown_{\phi} L
...
estimating \space \bigtriangledown_{\phi} J(\phi) : \\
\space sample \space \epsilon_1,\cdots,\epsilon_M \space from \space N(0,1) \\
\space \bigtriangledown_{\phi} J(\phi) \approx \frac{1}{M} \sum_j \bigtriangledown_{\phi} r(x_i,\mu_{\phi} (x_i) + \epsilon \sigma_{\phi} (x_i))
...
= \mathbb{E}_{\epsilon \sim N(0,1)} [log p_{\theta} (x_i \vert \mu_{\phi} (x_i) + \epslion \sigma_{\phi} (x_i) )] - D_{KL} ( q_{\phi} (z \vert x_i ) \parallel p(z) ) \\

\approx log p_{\theta} (x_i \vert \mu_{\phi} (x_i) + \epslion \sigma_{\phi} (x_i) ) - D_{KL} ( q_{\phi} (z \vert x_i ) \parallel p(z) ) 
...
### <mark style='background-color: #dcffe4'> What does the decoder learn? </mark>
...
![vqvae_slide_example1](/assets/images/ae_to_vae/vqvae_slide_example1.png){: width="80%"}
...
![vqvae_slide_ar](/assets/images/ae_to_vae/vqvae_slide_ar.png){: width="80%"}
