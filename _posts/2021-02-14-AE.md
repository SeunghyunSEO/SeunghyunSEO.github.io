...
PCA가 non-parametric한 방법이며 (학습할 필요가 없음) 일반적으로 잘 작동하는 알고리즘인데 "왜 AE를 알아야 하는가?"같은 의문점이 드실 수 있지만, 현대의 수 많은 컴퓨터비전, 음성인식, 자연어 처리 등의 분야에서  비선형성을 무기로 한 심층 신경망이 선형적으로만 이루어진 퍼셉트론을 압도하며 놀라운 결과들을 보여줬다는 데에서 AE는 알아 둘 필요가 있을 것 같습니다.  
...
이제 수식을 가지고 글을 전개하기 위해 몇 가지 Notation을 아래와 정의하고 이야기하도록 하겠습니다.  
...
(Reconstruction Error term인 이유가 이해가 안가시는 분들을 위해서 덧붙혀 설명하자면, 가우시안 분포를 출력 분포로 가정한 경우 (회귀문제 같은 경우) 그때의 likelihood를 최대화 하는 것은  `Mean Squared Error (MSE) Loss`를 최소화 하는 것과 같은데, 마찬가지로 우리는 생성한 이미지의 픽셀 값 하나하나를 전부 원본 데이터와 비교해서 그 loss가 최소화 되게끔 학습 하기 때문에 Reconstruction Error term이라고 부릅니다.)
...
이 수식이 의미하는 바는 즉, 우리가 $$x \rightarrow z$$인 deterministic한 함수를 알고 있다면, `Jacobian` 인 $$\frac{df}{dz}$$의 `determinant`값을 계산하는 것 만으로도 $$p(z) \rightarrow p(x)$$ 처럼 분포를 바꿀 수 있다는 겁니다.
...
CVAE는 VAE의 아이디어와 크게 다르지 않습니다. 가장 큰 차이는 $$p(x)$$ 를 모델링 하는 것에서 $$p(y \vert x,z)$$로 바뀌었다는 건데요,
마찬가지로 $$p(y \vert x)$$ 자체는 굉장히 복잡한 분포이겠지만, $$p(y \vert x,z)$$와 $$p(z)$$는 그렇지 않을 것이며 VAE가 그랬던 것 처럼 이 두개의 다루기 쉬운 분포의 곱으로 $$p(y \vert x)$$를 나타낼 수 있을 겁니다.
...
- CVAE
  ...
  - Decoder : $$p_{\theta}(y_i \vert x_i,z)$$
  - ELBO : $$L_i = \mathbb{E}_{z \sim q_{\phi}(z \vert x_i,y_i)} [log p_{\theta}(y_i \vert x_i,z) + log p(z)] + H( (q_{\phi} (z \vert x_i,y_i) ) )$$
  - Final Objective : $$ \hat{\theta},\hat{\phi} = arg max_{\theta,\phi} ( \sum_{i=1}^{N} [ log p_{\theta} (y_i \vert (\mu_{\phi} (x_i,y_i) + \epsilon \sigma_{\phi} (x_i,y_i)), x_i ) - D_{KL} ( q_{\phi} (z \vert x_i,y_i ) \parallel p(z) ) ] ) $$
- VAE
...
![vae_vs_cvae](/assets/images/ae_to_vae/Tutorial on Variational Autoencoders_fig3.png)
*Fig. VAE*

![vae_vs_cvae2](/assets/images/ae_to_vae/Tutorial on Variational Autoencoders_fig5.png)
...
![cvae4](/assets/images/ae_to_vae/cvae4.png)
*Fig.*

![cvae5](/assets/images/ae_to_vae/cvae5.png)
*Fig.*
