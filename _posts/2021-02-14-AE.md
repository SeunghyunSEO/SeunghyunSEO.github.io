...
- CVAE
  ...
  - Final Objective : $$ \hat{\theta},\hat{\phi} = arg max_{\theta,\phi} ( \sum_{i=1}^{N} [ log p_{\theta} (y_i \vert (\mu_{\phi} (x_i) + \epsilon \sigma_{\phi} (x_i)), x_i ) - D_{KL} ( q_{\phi} (z \vert x_i,y_i ) \parallel p(z) ) ] ) $$
- VAE
