...
사실 우리가 위의 그림에서 정말 원하는 것은 `인코더(Encoder)` 부분입니다.
그러니까 고차원의 입력 데이터를 저차원으로, 정보를 압축(Encoding)하는 부분 인거죠.

그렇기 때문에, 우리는 원래는 디코더가 필요 없으나, `인코더를 학습 시키기 위해서` 입력 데이터 x를 네트워크에 통과시켜 다시 x를 만들어내게끔 하는 비지도 학습 방법으로 네트워크를 학습하기 위해서 다시 저차원의 압축된 정보를 원래의 차원으로 복원시켜줄 디코더(Decoder)를 덧붙혀서 학습을 시키는 겁니다.
...
이 수식이 시사하는 바는 큰데, 이는 $$L_i(p,q_i)$$를 유도하는 또 다른 방법이기도 하며, $$log p(x_i)$$와 lower bound $$L_i$$간의 괴리 (error)가 얼마나 크고 작은지를 알 수 있기 때문입니다.


만약 두 진짜 분포와 근사 분포간의 KLD가 작다고 생각하면, $$L_i$$가 $$log p(x_i)$$를 굉장히 잘 근사 한다는 걸 알수 있겠죠? (극단적으로 0이면, 둘은 동일함)
이러한 


KLD는 언제나 양수죠, 그리고 KLD와 $$L_i$$의 합이 $$logp(x)$$입니다.
즉 우리는 $$L_i$$를 최대화 하는 것이 즉 KLD를 최소화 한다는걸 알 수 있습니다. 
KLD가 최소화 된다는 것은? 그렇습니다. $$q(z_i)$$와 $$p(z \vert x_i)$$가 0에 가까워지는, 즉 두 분포가 동일해 지는 결과를 가져오게 되고,
이는 우리가 원했던 `다루기 쉬우나 실제 분포를 잘 반영하는 근사 분포`를 얻을 수 있게 해줍니다.

![elbo_kld](/assets/images/ae_to_vae/elbo_kld.png)
*Fig. $$L_i$$를 최대화 하는 것은, 실제 분포와 근사 분포의 KLD를 최소화 하는 것이다. (Notation이 다른 이유는 후에 다시 서술하도록 하겠습니다.)*

다시 한 번, $$L_i(p,q_i)$$는 아래의 조건을 만족하기 때문에,
...
$$L_i(p,q_i)$$를 최대화 하는 것이 

결국 우리가 원하는 `VAE의 목적 함수 (Objective)`인 $$L_i(p,q_i)$$를 유도하는 또 다른 방법이기도 하며, 또 다른 해석 이기도 하기 때문입니다.
...
결국 Lower Bound, $$L_i (p, q_i)$$를 $$q_i$$ 에 대해서 최대화 하는 것이 KL-divergence 를 최소화 하는, 즉 실제 분포와 근사 분포를 유사하게 만드는 것이 된다는 .


![kld2](/assets/images/ae_to_vae/kld2.png)
*Fig.*

![kld3](/assets/images/ae_to_vae/kld3.png)
*Fig.*
