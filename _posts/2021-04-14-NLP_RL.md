...
*Fig. Seq2Seq Model for Neural Machine Translation*

![seq2seq2](/assets/images/NLP_RL/seq2seq_attention.png)
*Fig. Seq2Seq Model with attention mechanism*


<center>
$$ J(\theta) = - \mathbb{E}_{x~P(x)}[\mathbb{E}_{y~P(y)}[logP(y|x;\theta)]]$$
$$ \approx - \frac{1}{N} \sum_{i=1}^{N} \sum{y_{1:m} \in Y} P(y_{1:m}|x_{1:n}^i) logP(y_{1:m} | x_{1:n}^{i};\theta) $$
$$ - \frac{1}{N} \sum_{i=1}^{N} logP(y_{1:m} | x_{1:n}^{i};\theta) $$
$$ - \frac{1}{N} \sum_{i=1}^{N} \sum_{t=0}^{m} logP(y_{1:m} | x_{1:n}^{i}, y_{<t}^{i};\theta) $$
$$ where \space P(y_{1:m}|x_{1:n}^i) = 1 $$
</center>

이는 `경사 하강법 (gradient descent)`을 방식으로 최적화를 하게 됩니다.

<center>
$$ \theta \leftarrow \theta - \gamma \bigtriangledown_{\theta} J(\theta) $$
$$ \theta \leftarrow \theta + \gamma \frac{1}{N} \sum_{i=1}^{N} \sum_{t=0}^{m} \bigtriangledown_{\theta} logP(y_{1:m} | x_{1:n}^{i}, y_{<t}^{i};\theta) $$
</center>
