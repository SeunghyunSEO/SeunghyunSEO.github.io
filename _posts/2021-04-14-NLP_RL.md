...
여기서 우변의 첫째 항인 ($R_{t+1} + \gamma max_a Q(S_{t+1},a)$)는 타겟 가치 함수(Target Value Function) 이 되고, 두번째 항인 ($Q(S_t,A_t)$)는 현재의 가치 함수(Current Value Function) 이 되어 이 두 가치 함수의 차이를 줄이면서 학습하게 되면 우리는 결국 올바른 가치 함수를 얻게 된다는 것이 바로 큐러닝 입니다.
