...
<center>
$$ \bigtriangledown_{\theta} J(\theta) = \sum_{s \in S} d(s) \sum_{a \in A} \bigriangledown_{\theta} \pi_{\theta} (s,a) R_{s,a} $$
$$ = \sum_{s \in S} d(s) \sum_{a \in A} \pi_{\theta} (s,a) \bigtriangledown_{\theta} log \pi_{\theta} (s,a) R_{s,a} $$
$$ = \mathbb_{\pi_{\theta}} [ \bigtriangledown_{\theta} log \pi_{\theta} (a|s) r ] $$
</center>
...
<center>
$$ \bigtriangledown_{\theta} J(\theta) = \mathbb_{\pi_{\theta}} [ \bigtriangledown_{\theta} log \pi_{\theta} (a|s) Q^{\pi_{\theta}} (s,a) ] $$
</center>
...
이 수식에서 $log \pi_{\theta} (a|s)$가 의미하는 것은 어떤 상태 $s_t$가 주어졌을 때, 정책 파라메터 $\theta$ 상에서의 확률 분포에서 샘플링되어 선택된 행동이 $a_t$일 확률 값입니다.
따라서 위의 경사 상승법은 $log \pi_{\theta} (a|s)$를 최대화 하는 것으로, 높은 보상을 리턴하는 $a_t$의 확률을 높이도록 함으로써(즉 해봤더니 반응이 더 좋은 행동을 더 선택하게 확률을 높히게 끔 파라메터를 업데이트 함) 학습을 하는 것입니다.
