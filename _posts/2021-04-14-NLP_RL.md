...
입력 문장(Source Sentence)을 인코더에 넣어 얻어낸 인코더 출력을 디코더에 넣어 타겟 토큰(Target Token)을 하나씩 만들어 내어 최종적으로 토큰들을 나열한 것을 타겟 문장(Target Sentence) 라고 하는데, 여기서 어느 시점까지 만들어진 토큰을 
