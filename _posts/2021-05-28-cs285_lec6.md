...
Taxonomy에 보시면 Actor-Critic 알고리즘에는 이를 발전시킨 `Advantage Actor-Critic, A2C`, 하나의 에이전트가 아닌 여러 에이전트로 부터 학습하는 비동기식 알고리즘인 `Asynchronous Advantage Actor-Critic, A3C` 등도 있는데 이번 강의에서도 이 개념들을 다루고 있습니다.
...
여기서 V함수 대신 Q함수를 근사해서 사용하면

$$
A^{\pi} (s_t,a_t) = \sum_{t'=t}^{\infty} \gamma^{t'-t} r(s_{t'},a_{t'}) - Q_{\phi}^{\pi}(s_t,a_t)
$$
...
&
\bigtriangledown_{\theta} J(\theta) \approx \sum_i (\sum_{t=1}^T) \bigtriangledown_{\theta} log \pi_{\theta} (a_t^i \vert s_t^i) (\sum_{t'=t}^T r(s_t^i,a_t^i))
& \scriptstyle{\text{ Vanilla Policy Gradient Form 1 } }  \\

&
\bigtriangledown_{\theta} J(\theta) \approx \sum_i (\sum_{t=1}^T) \bigtriangledown_{\theta} log \pi_{\theta} (a_t^i \vert s_t^i)   \color{red}{Q(s_{i,t},a_{i,t}))}
& \scriptstyle{\text{ Vanilla Policy Gradient \color{blue}{Form 2} } }  \\
...
& \scriptstyle{\text{ Vanilla Policy Gradient with Baseline Form 1} }  \\
...
& \scriptstyle{\text{ Vanilla Policy Gradient with Baseline \color{blue}{Form 2} } }  \\
...
& \scriptstyle{\text{ Actor-Critic Form 1} }  \\
...
& \scriptstyle{\text{ Actor-Critic \color{blue}{Form 2} } }  \\
...
& \scriptstyle{\text{ \color{blue}{V Function} as Baseline } }  \\
...
& \scriptstyle{\text{ \color{blue}{Q Function} as Baseline Form 1 } }  \\
...
& \scriptstyle{\text{ \color{blue}{Q Function} as Baseline \color{blue}{Form 2} } }  \\
