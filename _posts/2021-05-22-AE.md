...
`적대적 생성 신경망 (Generative Adversarial Networks, GAN)`은 정해진 학습 데이터들에 대해서 $$z$$가 어떤 이미지로 매핑될 지를 log-likelihood를 최대화 하는 방향으로 (ELBO를 최대화) 지도 학습 하는 것과 달리, 모든 진짜같은 이미지들에 대해서 두 네트워크가 경쟁하듯 학습되는, 딱히 뚜렷한 Objective가 없는 (not a clear objective function to optimize) 방법론 입니다.
