...
$$z$$로부터 이미지를 만들어내는 `생성자 (Generator)`, 그리고 학습 데이터셋에 있는 이미지는 `True`라고 구분하고 $$z$$로부터 만들어진 이미지는 `False` 라고 구분하는 `판별자 (Discriminator)`라고 
하는 두 가지 네트워크를 아래와 같은 Objective로 학습합니다.
