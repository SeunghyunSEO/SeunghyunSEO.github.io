...
(그림이 조금 이상합니다 ㅠㅠ 그림을 가져온 UCLxDeepmind 강의에서도 lecturer가 그림이 이상하다고 양해 바란다고 했네요, 감안하고 느낌만 생각해주시길 바랍니다.)
...
*Fig. 29. MNIST 데이터의 embedding space, 사실 더욱 고차원인 데이터를 3차원에 매핑해서 보여준 것 뿐이고 원래는 더 고차원임, 출처 : [link](http://projector.tensorflow.org/)*
...
- <mark style='background-color: #fff5b1'> Matrix Multiplication = Correlation </mark>
...
*Fig. 30. 매트릭스 곱은 곧 입력값과 해당 뉴런과의 상관관계를 나타내는 것이다.*
...
*Fig. 31. 숫자를 나타내는 요소들이 있는지 검사하는 의미를 지니고 있는 hidden neuron들*
