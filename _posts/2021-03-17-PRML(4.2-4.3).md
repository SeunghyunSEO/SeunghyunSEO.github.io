...
PRML에서는 위의 그림 말고는 공분산이 다른 경우에 대한 자세한 설명이 나와있지 않다. 하지만 다른 패턴 인식 책에는 잘 나와 있는 경우가 많으니 참고할 것. (예로 Duda 패턴인식 2장)
...
\\( \pi \\) 구하기 위해 로그 가능도 함수를 \\( \pi \\) 에 대해 미분하여 관련된 항목만 모으고, 이 값을 0으로 놓고 \\( \pi \\) 를 구하면 된다. 
