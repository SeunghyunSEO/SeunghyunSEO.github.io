...
이제 본격적으로 강화 학습의 기본적인 알고리즘들에 대해서 알아보도록 하겠습니다. (Policy를 학습하는지가 목적)
...
![traject1](/assets/images/CS285/lec-4/traject1.png)
*Additive. Fig.*

이 중에는 더 좋은 Trajectory가 있을 수 있겠죠?

![traject2](/assets/images/CS285/lec-4/traject2.png)
*Additive. Fig.*

우리는 뭐가 좋은 Trajectory인지를 알기 위해서 각 Trajectory가 좋고 나쁜 걸 평가 (evaluate) 할 수 있고 (`초록색 박스`),

![traject3](/assets/images/CS285/lec-4/traject3.png)
*Additive. Fig.*

좋다고 생각하는 Trajectories이 등장할 확률을 높히는 방향으로 (make good trajectory be more likely and bad trajectory be less likely) 파라메터를 학스바면 됩니다 (`파란색 박스`). 여기서 좋고 나쁜 걸 평가 (evaluate) 하는 것은 단순히 매 상태 (state) 마다의 보상의 합 (sum of rewards)을 구하면 됩니다.

![traject4](/assets/images/CS285/lec-4/traject4.png)
*Additive. Fig.*
...
`초록색 박스`는 *Slide. 19.*에서 보는바와 같이 정말 단순히 보상의 합에 대한 기대값 이며, 이렇게 측정한 값이 얼마나 우리의 policy가 좋은가를 나타내고, `파란색 박스`는 Policy 파라메터에 대해 경사 하강법 (Gradient Descent)을 하는 거라고 생각하시면 됩니다.
...
이러한 방법론은 `초록색 박스`에서 `Model`을 학습하는데, 다시 말해 뭔가 다른 `Neural Network (NN)`, $$f_{\phi}$$을 학습한다는 겁니다.
