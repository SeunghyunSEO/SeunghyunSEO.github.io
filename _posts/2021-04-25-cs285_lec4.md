...
여기서 Lecturer는 질문을 하나 던지는데, 바로 "`State-Action Marginal`$$p(s_t,a_t)$$ 가 어떠한 분포 (Stationary Distribution)으로 수렴할 수 있느냐?" 입니다.

```
What is Stationary Distribution?

Markov Chain은 부루마블 ... 그래프 위에서 매 턴마다 정해진 확률에 따라 다른 노드로 이동하는 것과 같습니다.
...(중략)
...
결과적으로  `Stationary Distribution`이 존재하고, $$\mu$$라는 상태가 $$T \mu$$ 에서 이 Markov Chain을 앞으로 여러번 시행시키면  $$T \mu$$ 가 되고, 그러므로 $$t \rightarrow \infty$$면 State-Action Marginal에 기대값을 취한 것의 합인 $$\sum_{t=1}^T \mathbb{E}_{(s_t,a_t) \sim p_{\theta}(s_t,a_t)}[r(s_t,a_t)]$$이 `Stationary Distribution terms`에 의해서 dominate되고 맙니다. (???)
