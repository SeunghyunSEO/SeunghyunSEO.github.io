...
우리는 지금 State와 Action을 묶어서 생각하는 Augmented Markov Chain을 생각하기로 했기 때문에 `S1,S2,S3,S4` 4차원의 State Space와, `상,하,좌,우`로 움직일 수 있는 마찬가지로 4차원의 Action Space가 있다면 $$p(s_t,a_t)$$는 16차원이 됩니다. 그러므로 Transition Probability $$T$$도 $$\mathbb{R}^{16 \times 16}$$이 .
