...
> $$x = (x_1 , . . . , x_T )$$ $$\rightarrow$$ 입력 벡터들(전부 합쳐서 입력 매트릭스 = Mel Spectrogram)이 됩니다. 
> $$y = (⟨sos⟩, y_1 , . . . , y_S , ⟨eos⟩)$$ $$\rightarrow$$ 우리가 추론할 정답입니다. 
> $$y_i ∈ \{ a, b, c, · · · , z, 0, · · · , 9, ⟨space⟩, ⟨comma⟩, ⟨period⟩, ⟨apostrophe⟩, ⟨unk⟩ \}$$ $$\rightarrow$$ 출력 토큰의 분포 or 단어 사전입니다.
> $$⟨sos⟩$$ $$\rightarrow$$ 단어 사전에 들어있는 특별한 토큰으로 디코딩 시 '자 이제 디코딩 시작할거다, 이걸 받아서 이 음성을 받으면 어떤 값을 처음으로 뱉어야하는가?'를 뱉게 할겁니다.
> $$⟨eos⟩$$ $$\rightarrow$$ 문장의 끝을 알립니다. 이 토큰을 뱉으면 디코딩을 종료하게 할겁니다. 
