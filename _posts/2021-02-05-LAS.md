...
핵심은 
```
Representation Vector 하나에 모든 정보를 담지 말고, 입력사이즈 만한 Representation Memory에서 Attention을 이용해 
'이번 토큰을 만들어 낼 때는 입력값에서 어느 부분의 정보를 가져오면 될까?(어디를 집중!(Attention)하면 될까?'라는 것을 고려해서 
매 번 디코딩 할 때 마다 필요한 정보만 뽑아오자 
```
라는 겁니다.
