...
이 음성의 길이가 2초일 경우 sampling rate가 8000이라면 16000차원인데, 이를 입력으로 사용해서 출력해야할 글자는 19차원(글자)입니다. 이렇게 입력과 출력의 차이가 맞지 않아, "어디서부터 어디까지가 '안'이고, 어디서부터 어디까지가 '녕'인가?"를 매칭하기 힘든 문제를 Miss-Alignment 문제라고 합니다.
...
최종 출력 : "c a t"
```
`CTC Decoding Example` : 위에는 두개의 디코딩 예시만 들었지만 사실 위와 같은 규칙으로 최종 출력을 만들 수 있는 경우의 수는 많으며, CTC는 이러한 가능한 모든 alignment를 잠재 변수(latent varialbe)로 생각하여 학습하는 모델입니다.
...
이들을 바탕으로 디코더에서 토큰을 하나를 디코딩 하면, 그렇게 만들어진 토큰들을 다음 디코딩 할 때 정보로 주어 또 디코딩을 하고 ... '디코딩이 끝났다'는 의미의 <EOS> 토큰을 뱉을 때 까지 계속 디코딩을 하는 
Autoregressive Decoding Model입니다. 
...
물론 위에서 언급한 네번 문제처럼, 어텐션을 사용한 모델이 음성인식이 요구하는 단조로운 (Monotonic) alignment를 만들어내지 못하는 것은 아닙니다. 다만 초기 학습에 어렵다는 것이죠.

![alignment](/assets/images/rnnt/alignment.png){: width="80%"}
*Fig. 잘 학습된 Seq2Seq with Attention Model의 단조로운 (Monotonic) Alignment*


하지만 네번 문제를 해결하기 위해서 CTC와 Attention을 결합한 기법이 제안되기도 했는데요,  
이러한 모델은 아래의 그림과 같고, 이렇게 네트워크를  CTC Loss가 학습 초기 Monotonic Alignment를 배우게끔 하여 더욱 전체 모델을 잘 학습할 수 있게 합니다. 
