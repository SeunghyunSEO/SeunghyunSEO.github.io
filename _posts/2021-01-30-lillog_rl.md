...
로봇은 결과적으로 누적 보상을 최대화 하는방향으로 액션을 취해야 할 것입니다. (여기서 누적 보상이라고 하는 이유는, 현재 보상만 최대화 하는 방향으로 행동하는게 미래에는 안좋을 수 있기 때문이죠, 예를 들어 우리가 지금 강화학습을 공부하는게 고통스러울 지언정, 미래엔 도움이 될 것이고 (그렇겠죠...?), 지금 랩탑을 덮고 피시방에서 롤 하러 가는건 당장은 해피할 지 몰라도 미래에는...) <br>
...
- Does not know the model: 부족한 정보를 가지고 학습하는겁니다; **<span style="color:#e01f1f">do model-free RL</span>** 한다고 합니다. 혹은 try to learn the model explicitly as part of the algorithm. 앞으로 우리가 얘기할 것들은 대부분 Model-free 상황일겁니다.
...
아래의 용어들은 앞으로 보게될 수많은 RL 알고리즘들에서 마주치게 될테니 익숙해지시길 바랍니다ㅎ : <br>
...
[tmp](https://cs.stanford.edu/people/karpathy/reinforcejs/gridworld_dp.html)
