...
실제, real world에서 위에서 얘기한 것들은 가령 게임에서 고득점을 얻거나 상대방을 이기는 것 등을 목적으로 하는 로봇드를 예시로 들 수 있을겁니다.      
...
agent는 환경의 수많은 **states**(상태) ($$s \in \mathcal{S}$$)들 중 하나에 있을겁니다. (격자 구조 미로라면 미로의 한 칸, 바둑을 두는 상황이라면 바둑이 놓여진 어느 한 상황)
...
근데 agent가 어느 state에 도착할지는 **transition probabilities between states** ($$P$$)에 의해서 결정됩니다. (어떤 상태 $$s_11$$에서 $$s_12$$,$$s_13$$...등으로 갈 확률을 나타내는 테이블이 있어요)<br>
