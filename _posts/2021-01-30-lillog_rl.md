...
(stochastic하다는 것은 예를들어 $$\pi(a \vert s) = [상=0.12,하=0.48,좌=0.3,우=0.2]$$ 처럼 결과를 뱉는 다는 겁니다.)
...
future reward 라는 것은 아~주 미래에 얻게 될 보상으로 ```return```이라고도 합니다. 앞으로 얻게 될 보상들의 합이기도 하죠. ($$R_2 + R_3 + .... R_T$$), 하지만 미래에 얻게될 보상일 수록 ($$R_2 + \gammaR_3 + ...$$) 같이 됩니다. $$G_t$$를 시작지점 t 부터 해서 계산해볼까요?:
