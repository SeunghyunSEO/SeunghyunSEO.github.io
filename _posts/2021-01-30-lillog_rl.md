...
### <mark style='background-color: black'>(1/11)</mark> <mark style='background-color: #dcffe4'> Key Concepts </mark>
...
- Know the model: (환경, 어떻게 상호작용하는지 등)모든 정보를 다 알고 학습하는겁니다; model-based RL을 한다고 합니다. 우리가 환경을 정확히 알고 있을 때에는 (어떻게 작용하는지 명확히 암) 우리는 [Dynamic Programming](https://en.wikipedia.org/wiki/Dynamic_programming) (DP)을 통해서 최적 해(optimal solution)을 구해낼 수 있습니다. 
- Does not know the model: 부족한 정보를 가지고 학습하는겁니다; do model-free RL or try to learn the model explicitly as part of the algorithm. 앞으로 우리가 얘기할 것들은 대부분 Model-free 상황일겁니다.


```policy```(정책) $$\pi(s)$$은 agent가 어떤 상황에 처했을 때 어떤 행동을 하는게 최적인지를 with **<span style="color:#e01f1f">'Total Rewards'(겜 끝날때 까지  총 보상)를 최대화 한다는 목적으로</span>** 알려주는(확률분포로 알려줍니다) 함수입니다.
각각의 state는 가치 함수라고 하는 ```value function``` $$V(s)$$ 와 깊은 연관이 있는데, 이 가치 함수는 '현재 상태' 에서 '우리가 현재 해당 policy를 따라 행동을 함으로써 미래에 얻을 수 있는 ```future rewards```의 기대값을 예측하죠.   
다시 말하면, 가치함수는 **<span style="color:#e01f1f">'현재 상태가 얼마나 좋은가?'</span>** 를 정량화 해주는거죠.

정말 다 떼놓고 간단하게 말하면, 강화학습에서 우리가 학습하고자 하는 것은 이 두가지 ```policy function```, ```value function``` 라고 할 수 있습니다.
...
### (2/11) <mark style='background-color: #dcffe4'> Markov Decision Processes </mark>
...
### (3/11) <mark style='background-color: #dcffe4'> Bellman Equations </mark>
...
### (4/11) <mark style='background-color: #dcffe4'> Dynamic Programming </mark>
...
### (5/11) <mark style='background-color: #dcffe4'> Monte-Carlo Methods </mark>
...
### (6/11) <mark style='background-color: #dcffe4'> Temporal-Difference Learning </mark>
...
### (7/11) <mark style='background-color: #dcffe4'> Combining TD and MC Learning </mark>
...
### (8/11) <mark style='background-color: #dcffe4'> Policy Gradient </mark>
...
### (9/11) <mark style='background-color: #dcffe4'> Evolution Strategies </mark>
...
### (10/11) <mark style='background-color: #dcffe4'> Exploration-Exploitation Dilemma </mark>
...
### (11/11) <mark style='background-color: #dcffe4'> Deadly Triad Issue </mark>
