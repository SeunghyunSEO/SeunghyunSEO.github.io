...
& L = \mathbb{E}_{w \sim q(w \vert \theta)} [log p(y \vert x,w)] + \mathbb{E}_{w \sim q(w \vert \theta)} [ log p(w)] + H(q(w \vert \theta)) & \\

& L = \mathbb{E}_{w \sim q(w \vert \theta)} [log p(y \vert x,w)] - D_{KL} ( q(w \vert \theta) \parallel p(w) )  & \\
