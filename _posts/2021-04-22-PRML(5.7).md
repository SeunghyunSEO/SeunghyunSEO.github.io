...
연속적인 출력 분포를 가정하기 위해서 익숙한 가우시안 분포를 가정해보도록 하겠습니다.
...
여기에 posterior는 일반적으로 아래와 같은 관계식을 따르므로
...
오차 역전파 (Error Backpropagation)과 경사 하강법 (Gradient Descent)를 이용해서 $$w_{MAP}$$를 찾은 뒤, 음의 로그 사후 분포의 이차 미분 행렬을 바탕으로 우리는 지역적 가우시안 근사치를 얻을 수 있습니다.
...
우리는 앞서 4절에서 배운 것 처럼 posterior를 근사한 분포를 아래처럼 얻을 수 있다.
...
이고, 베이지안 방법론의 목적은 파라메터를 점 추정 하는것에 그치지 않고 (MLE나 MAP는 최대값인 파라메터 딱 하나만을 결과물로 취함), 모두 고려하는 것이기 때문에 (weight에 대한 uncertainty를 최대한 없앰 (?))
사후 분포를 주변화 (marginal) 한 수식은 아래와 같이 표현이 가능하고,

$$
p(t \vert x,D) = \int p(t \vert x,w) p(w \vert D, \alpha, \beta) dw
$$

우리는 라플라스 근사식을 통해서 $$\int$$속 복잡한 posterior를 근사 분포로 만들어 최종적으로 다음과 같은 수식을 얻을 수 있었습니다.
