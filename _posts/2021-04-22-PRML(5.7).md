...
$$
log  \space evidence = log (p( Y \vert X )) = log ( \int p(Y \vert X, w) p(w) dz ) \geq \int log ( p(Y \vert X, w) ) p(w) dw
$$

$$
log (p( Y \vert X )) = log ( \int p(Y \vert X, w) \frac{p(w)}{q(w \vert \theta) } q(w \vert \theta) dw ) \geq \int log ( p(Y \vert X ) \frac{ p(w) }{ q(w \vert \theta) } ) q(w \vert \theta) dw 
$$

$$
log (p( Y \vert X)) = \int log (p(Y \vert X,w)) q(w \vert \theta) dw - \int log (\frac{q(w \vert \theta}{p(w)} ) q(w \vert \theta) dw
$$

$$
log  \space evidence = log (p( Y \vert X)) = log (D) \geq  \mathbb{E}_{q(w \vert \theta)} [ log(p(Y \vert X, w))] - KL(q(w \vert \theta) \parallel p(w))
$$
