...
그러므로 여기서 약간은 근사 (무시, ignore)가 들어가야 되는데요, initial state distribution을 $$\theta$$에서 뽑는겁니다.
...
아래의 initial state distribution을 old policy로 바꿔서 `new policy`을 얻는 수식을 얻어냈죠.
