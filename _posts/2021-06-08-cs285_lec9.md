...
$$J(\theta')-J(\theta)$$는 아래와같이 써 볼 수 있는데요,
...
우리가 원하는 것은 이 두개의 gap을 최대한 크게 하는 정책으로 한스텝 나아가는 것이기 떄문에 $$\theta'$$에 관해서 위의 수식을 최대화 하는 것이 좋은 정책으로의 업데이트라는 것을 직감적으로 알 수 있습니다.

한편 이는 Policy Iteration과 닮았는데요, 정책 반복이 Advantage Function을 다 구하고, 이를 바탕으로 정책을 업데이트하는 것인데, 이와 크게 다르지 않은 흐름이기 때문입니다.
...
우리가 앞서 증명한 것은 RL objective를 maximize하는 것이 $$J(\theta')-J(\theta)$$를 maximize하는 것 (물론 Advantage는 old policy로부터, trajectory는 new policy로 부터)과 같다는 겁니다.

그리고 이 수식은 아래와 같이 바꿀 수 있는데요,
...
&
\mathbb{E}_{\tau \sim p_{\theta'}(\tau)} [ \sum_t \gamma^t A^{\pi_{\theta}}(s_t,a_t) ] = \sum_t \mathbb{E}_{s_t \sim p_{\pi_{\theta'}}(s_t)} [\mathbb{E}_{a_t \sin \pi_{\theta'} (a_t \vert s_t)} [\gamma^t A^{\pi_{\theta} (s_t,a_t)}] ] 
& \\
\end{aligned}
$$
...
&
\mathbb{E}_{\tau \sim p_{\theta'}(\tau)} [ \sum_t \gamma^t A^{\pi_{\theta}}(s_t,a_t) ] = \sum_t \mathbb{E}_{s_t \sim p_{\pi_{\theta'}}(s_t)} [\mathbb{E}_{a_t \sin \pi_{\theta} (a_t \vert s_t)} [ \frac{\pi_{\theta'} (a_t \vert s_t)}{\pi_{\theta} (a_t \vert s_t)} \gamma^t A^{\pi_{\theta} (s_t,a_t)}] ] 
& \\

\end{aligned}
$$

주의할 점은 여전히 state,$$s_t$$는 $$\pi_{\theta'}$$의 정책에서 샘플링되고 있고 $$(a_t \vert s_t)$$만 $$\theta$$로 부터 샘플링 되고 있다는 겁니다.
근데 여기서 하나 남은 문제점이 있는데요 $$\pi_{\theta'}$$가 뭔지 아직 모르는 상태에서 state를 샘플링 할 수 있을까요? 아닙니다.
