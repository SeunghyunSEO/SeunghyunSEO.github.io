...
우리가 $$p_{\text{mistake}}(s_t)$$가 뭔지 알 수 없기 때문에 적당히 bound를 아래와 같이 정할 수 없습니다.
...
물론 이느 deterministic case에 대한 얘기이기 때문에 다음으로 할 것은 general case에 대해서 이를 계산하는 겁니다. 
...
이 증명은 Policy Gradient 계열에서 policy를 안정적으로 학습하기 위해 그나마 최근에 (5,6년 됐죠) 제안돼 많이 각광받았던 `Trust Region Policy Optimization (TRPO)` 논문을 따른다고 합니다.

```
Trust Region Policy Optimization (TRPO)와 더불어 Proximal Policy Optimization (PPO)는 
Policy Gradient 계열의 방법론에서 중요한 위치를 차지하고 있는데,

두 방법론이 차이가 있을지언정 아이디어는 같습니다.
