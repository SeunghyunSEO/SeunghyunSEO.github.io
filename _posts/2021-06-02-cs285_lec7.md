...
본격적으로 `Policy Iteration`과 `Value Iteration` 등에 대해서 본격적으로 알아보기 전에, 이는 테이블 형식으로 가능한 state나 (state, action) tuple에 대한 가치(Value) 를 Dynamic Programming을 이용해 빠짐없이 구해내는 과정을 반복함으로써, 이를 바탕으로 최적의 policy를 결국 찾아내는 오래된 방법론임을 알아야 합니다. 게다가 이는 학습할 요소가 없는 강화 '학습'이 아닙니다 (물론 강의 중간중간 이를 발전시켜서 학습해야하는걸로 바꾸긴 합니다). 그럼에도 불구하고 왜 이 오래된 알고리즘들을 알아야하는가?에 대해서는 이렇게 Dynamic Programming을 통해서 솔루션을 구하는 아이디어가 현대의 강화학습에 지대한 영향을 끼쳤기 때문입니다.
...
이렇게 함으로써 평가하고 업데이트하는 시간을 줄인 것이죠.
...
```
Transition Dynamics를 모른다는 것은 "환경이 어떻게 구성되어있는지 모른다", 혹은 "Model을 모른다" 라고 할 수도 있는데요, 그러니까 우리가 어떻게 생겼는지 모르는 숫자가 6개인 주사위를 던질 때 1이나올 확률 6이나올 확률을 모른다는 겁니다. 그러니까 무수히 많이 주사위를 던져봐서 Transition Dynamics을 알아내던지 아니면 Transition Dynamics없이 이상적인 정책을 찾아내야합니다.
```
...
어떤 시점 t에서 $$(s_t,a_t,s'_t)$$ 이라는 걸 알고있기 때문에 `Transition Dynamics 분포`를 바탕으로 Expectation을 계산해내지 않아도 된다는겁니다.

(솔직히 좀 헷갈리네요 ... 잘 아시는분은 답변 달아주시면 감사드리겠습니다...!)
...
자, 우리가 이전에 Policy-based Method를 탈피해 Policy Iteration에 대해서 알아보고, 이를 개선해서 Value Iteration으로 넘어갔습니다.
...
기대값 수식을 $$max_{a'} Q_{\phi}(s'_i,a'_i)$$ 로 대체했고 여기서 중요한 점은 action들을 시뮬레이션 해 볼 필요가 없다는 겁니다.
그냥 가장 "최근에 얻은 정책 (latest policy)"을 돌려서 (roll-out, run) 얻은 샘플들을 사용하면 되는것이죠. 
이 때 중요한 점은  "최근에 얻은 정책" 대신에 다른 정책을 써도 된다는 겁니다. (나중에 자세히 다룰 듯 합니다.)
