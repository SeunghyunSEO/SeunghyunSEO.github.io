...
이번에는 Fitted Q-Iteration의 special case에 대해서 알아볼 건데요, 이는 굉장히 유명한 가치 기반 알고리즘입니다.
...
그러니까 `step 2` 에서 target을 평가하는데 무작정 argmax를 사용하는 것이 아니라 `랜덤성 (randomness)`을 섞어주는 것이 실제로는 중요합니다 (실제로 큰 state, 큰 action 을 가지는 문제들).
...
$$
\text{for any } V \text{ and } \bar{V}, \text{ we have } \parallel BV-B\bar{V} \parallel_{\infty} \leq \color{red}{ \gamma } \parallel V - \bar{V} \parallel_{\infty}
$$

핵심은 "우리가 만약 어떤 두 가지 vector $$V,\bar{V}$$를 가지고 있을 때 $$B$$를 두 벡터에 적용하는 것이 두 벡터가 (infinity norm 크기에 대해) 적어도 $$\gamma$$ 만큼 가까워지는 결과를 가져온다" 라는 겁니다. (여기서 infinity norm을 사용했다는 것과 우변에 $$\lambda$$가 곱해져 있다는 것에 주의)
...
$$
\parallel BV-V^{\ast} \parallel_{\infty} \leq \color{red}{ \gamma } \parallel V - V^{\ast} \parallel_{\infty}
$$
...
하지만 문제는 "Non-Tabular form의 경우에도 그런가?" 인데요, 우리가 푸는 대부분의 문제는 Non-Tabular form, 즉 Neural Network로 Value를 근사한 경우이기 때문에 이를 이해하는것이 중요합니다. (Fitted Value Iteration, ...)
...
2. set $$\phi \leftarrow arg min_{\phi} \frac{1}{2} \sum_i \parallel V_{\phi}(s_i) - y_i \parallel $$

Fitted Value Iteration은 Regression이 추가됐죠. step2는 $$\phi$$에 관해서 arg min 문제를 푸는 겁니다.
여기서 step 1은 Value Iteration에서와 같이 Bellman Backup으로 표현할 수 있는데, step 2는 Neural Network를 학습하는거죠, 이를 과연 어떻게 수학적으로 표현할까요?

먼저 우리는 Supervised Learning에 대해 생각해봐야 되는데요, 이는 우리가 Value Function을 뉴럴 네트워크로 모델링 했기 때문에 현재 네트워크 구조에서 가능한 파라메터의 경우의 수를 모두 포함하는 continuous Value Function set (hypothesis set 이라고 보통 함) 을 가지고 있으며 Supervised Learning을 통해서 네트워크를 학습해 최적해를 찾는 것은 이 set중 최적의 값을 찾는다는 겁 됩니다.

여기서 이 set을 $$\Omega$$라고 하면 Regression Error를 최소화 하는 파라메터를 찾는 것은 아래와 같이 나타낼 수 있는데요,

$$
V' \leftarrow arg min_{V' \in \Omega} \sum \parallel V'(s) - (BV)(s) \parallel^2
$$

여기서 Target Value는 step 1 이므로 당연히 BV로 표현할 수 있습니다.
종합하면 Fitted Value Iteration은 반복적으로 새로운 Value Function $$\color{red}{V'}$$를 set $$\Omega$$ 안에서  가장 작은 Squared Error $$\sum \parallel V'(s) - (BV)(s) \parallel^2$$ 값을 리턴하는 파라메터를 찾는 것이라고 할 수 있죠.
여기서 BV는 이전 Value Function을 의미합니다. 

즉 우리가 이런 Supervsied Learning을 하는것은 old V가 있고, 가능한 뉴럴 네트워크로 표현되는 set을 직선으로 나타낼 때 ($$\Omega$$는 직선의 모든 점들이 되는데, 그렇다고 이게 모든 가능한 Value Function을 나타내지는 않습니다.)

![sl_1](/assets/images/CS285/lec-7/sl_1.png){: width="60%"}
*Fig.*

우리가 여기서 BV를 하면 이 직선을 벗어나게 되고 (더이상 $$\Omega$$에 있지 않습니다.)

![sl_2](/assets/images/CS285/lec-7/sl_2.png){: width="60%"}
*Fig.*

우리가 step 2의 Supervised Learning을 하면 다시 $$\Omega$$ 내에 있는 어떠한 점으로 projection을 하는게 되는겁니다.

![sl_3](/assets/images/CS285/lec-7/sl_3.png){: width="60%"}
*Fig.*
...
&
\text{where B is } BV = max_a r_a + \gamma T_a V
& \\
\end{aligned}
$$
...
&
\text{where } B \text{ is } BV = max_a r_a + \gamma T_a V
& \\

&
\text{where } \Pi \text{ is } \Pi V = arg min_{V' \in \Omega} \frac{1}{2} \sum \parallel  V'(s) - V(s) \parallel^2
& \\
\end{aligned}
$$

여기서 Fitted Value Itearion에는 새로운 Operator $$\Pi$$가 추가된 것을 볼 수 있는데요, 이는 Projection Operator로 l2 norm 에 대해서 $$\Omega$$라는 곳으로 어떤 벡터를 사영시키는 operator입니다. $$\Pi V$$는 $$\Omega$$ 내에서 step 2의 Objective 의 argmin이 됩니다. 

여기서 $$\Pi$$ 또한 `contraction`이라고 하는데요, 왜냐하면 어떤 벡터를 l2 norm 하에서 사영시키면 그 점들은 가까워 질 수 밖에 없기 때문이라고 합니다.
...
$$\Pi B$$는 contraction이 아니라는 겁니다.
이게 둘 다 contraction인 연산이라서 처음에는 이상해보일 수 있지만 이 두가지가 다른 norm에 대해서 contraction이라서 $$\Pi B$$는 contraction이 아니게 된다고 합니다.

요약하자면 Value Iteration 에 대해서는 optimal value로 반복적인 연산을 하면 다가가는게 보장되어 있지만, 뉴럴 네트워크를 Supervised Learning으로 피팅하는 Fitted Value Iteration 은 보장되어 있지 않다는 겁니다. 

![tabular_vs_nontabular](/assets/images/CS285/lec-7/tabular_vs_nontabular.png)
*Fig.*
...
그렇다면 `Fitted Q-Iteration` 에서는 어떨까요?
...
&
\Pi : \Pi Q = arg min_{Q' \in \Omega} \frac{1}{2} \parallel Q'(s,a) - Q(s,a) \parallel^2 
& \\

&
\text{fitted Q-iterattion is } Q \leftarrow \Pi B Q 
& \\
\end{aligned}
$$

마찬가지로 $$B, \Pi$$는 각각 $$\infty$$-norm과 $$l_2$$-norm 에 대해서 contraction 이지만 둘 다 적용할 경우는 아니겠죠.

![slide26](/assets/images/CS285/lec-7/slide26.png)
*Slide. 26.*

여기서 많은 분들이 "왜 그냥 regression을 gradient descent로 최적화 하는거처럼 생겼는데 안된다는거야?" 라는 의문이 드실 수 있다고 하는데요, `Q-Learning 은 사실 잘 정의된 Objectvie에 대해 gradient descent를 하는건 아니다` 라고 교수님은 말합니다. 
왜냐면 Q-Learning 그자체에서 target은 Q-Value에 의존 여기서 target이 $$\gamma max_{a'} Q_{\phi}(s'_i,a'_i)$$ 이기 때문에 target 쪽에는 그래디언트가 흐르지 않는다고 합니다. 그렇기 때문에 converge 하지 않을 수 있다고 하네요 (?). 
여기서 target에 그래디언트가 흐르게 알고리즘을 수정함으로써 gradient descent로 알고리즘을 바꿀 수 있다고 하는데요, 이렇게 몇가지 테크닉들을 사용해서 그래디언트가 흐르게 해주는 `Residual Algorithm` 이라고 하지만 poor numerical property를 가지기 때문에 실제로는 잘 안된다고 합니다. 오히려 여태까지 말한거처럼 Fitted Q-Iteration, Fitted Value Iteration, Q-Learning등이 수렴성이 이론적으로 증명되지는 않지만 optimal value 비스무리한 값으로 Residual Algorithm 보다 더 잘 수렴할 가능성이 실제로는 존재한다고 합니다.
...
그리고 우리가 Lecture 6 에서 다뤘던 `Actor-Critic`도 사실은 같은 이유로 수렴이 보장되지 않는다고 합니다.
...
마지막 슬라이드는 review 입니다.



정말 긴 글인데 끝까지 봐 주신 분들 모두 감사드립니다.
