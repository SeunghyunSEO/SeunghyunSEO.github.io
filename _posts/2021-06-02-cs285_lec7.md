...
위에서 말한 special case란 online case인데요, `step 1`을 한번만 하는데요, 즉 action을 한번만 하고, 그 결과 transition도 한번 합니다 ( $$ s_i, a_i, r_i, s'_i$$ ). 따라서 step 2도 한번만 하게 하고 step 3 에서도 한번의 gradient step만 전진합니다.
이는 Actor-Critic의 online버전과도 같습니다.

General case와 special case의 비교는 *Slide. 17.*잘 나와있는데요,

주의하실 점은 step 3 가 아래처럼 표현되어 있다는 겁니다. 
크게 원래의 수식과 다른건 아니고, target $$y_i$$을 수식에 직접 대입하고 체인룰을 통해서 전개해 gradient descent을 표현한 겁니다.
...
이러한 special case를 우리는 `Online Q-Learning` 이라고 하며 1989년에 Watkins가 제안했다고 해서 [Watkins Q-Learning](http://citeseerx.ist.psu.edu/viewdoc/download;jsessionid=963E6386AB74F0CC72C44910D1B0E2E6?doi=10.1.1.466.7149&rep=rep1&type=pdf) 이라고도 합니다.
...
Tabular Value Iteration은 간단했습니다.
...
3. 반복
```
우리가 궁금한 것은, "이게 수렴하는가? 수렴하면 어디로 수렴하는가?" 인데요,
...
여기서 max operator는 모든 state마다 적용됩니다.
이를 `Bellman Backup`이라고 하는데요, 이는 원래의 Value Iteration 을 표현한 것으로,
...
이렇게 표현한 데는 이유가 있을텐데요, 이를 통해서 우리가 알 수 있는 것은

Optimal Policy를 나타내는 $$V^{\ast}$$가 $$B$$의 `fixed point`가 된다는 겁니다.
그러니까 $$V^{\ast}$$를 알면 optimal policy, $$\pi^{\ast}$$를 알 수 있는데 이게 B와 관련있는 거죠.
...
이제 우리가 보여야할 것은 "$$V^{\ast}$$가 항상 존재하는가? 그리고 BV를 반복적으로 수행 하면 그 지점으로 가는가?" 입니다.
...
의미는 "우리가 만약 두 가지 vector $$V,\bar{V}$$를 가지고 있을 때 $$B$$를 두 벡터에 적용하는 것이 두 벡터가 가까워지는 (infinity norm 크기에 대해) 결과를 가져온다" 라는 겁니다.

Bellman Operator를 적용하는 것이 그런 결과를 가져오는가에 대한 증명이 그렇게 어렵지는 않으나 본 강의에서 다루지는 않고, 여기서는 그럴 경우 value iteration이 수렴한다는 의미를 가지는가? 에 대해서만 간략하게 알아보는데, 
이는 $$\bar{V}$$를 B의 fixed point $$V^{\ast}$$로 바꾸면 성립합니다.
