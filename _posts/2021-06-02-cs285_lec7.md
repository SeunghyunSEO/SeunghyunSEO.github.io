...
*Fig. $$V^{\pi}_1$$를 모든 상태마다 계산하는 과정, 위의 figure에 적혀있는 수식은 6번째 타일 (모든 타일이 state인 것)에 대한 value를 계산하는 과정이다.*
...
$$<3,3>$$ 입장에서 max가 되는 행동은 `right`이 되기 때문에 right이라는 행동을 취했을 때 우리가 다음 state가 될 수 있는 후보군은 `자기 자신, 오른쪽, 아래`가 되고, 그렇게 될 확률들이 각각 0.9로 동일하다고 생각할 때 ( $$\sum$$ `<3,3>에서 다른 state로 이동할 확률`=( 각각 $$0.8,0.1,0.1$$) $$\odot$$ `현재 state 보상`($$=0$$) + `다음 state 가치`( 각각 $$1, 0, 0$$) 가 되는겁니다.
 
(여기서 $$\odot$$는 element-wise product)
