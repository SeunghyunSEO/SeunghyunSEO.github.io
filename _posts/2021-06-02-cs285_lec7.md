...
다시, DP를 이용해 최적의 policy를 찾는 `Policy Iteration`을 우리는 policy를 제거하고 직접적으로 value를 계산하는 `Value Iteration`으로 발전시켰고 여기서 state 수가 너무 많은 경우를 고려해 뉴럴 네트워크로 디자인한 `Fitted Value Iteration`에 대해 알아봤습니다. 그리고 여기서 Transition Dynamics를 모르는 경우 (Unknown MDP)에 대한 대안으로 V대신 Q를 사용하려고 합니다.
