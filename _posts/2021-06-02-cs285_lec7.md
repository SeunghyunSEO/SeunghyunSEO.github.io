...
여기서 또 $$ max_{a_{i'}} Q_{\phi} (s'_i,a'_i) $$를 rollout 해볼 수 있는데요, 우리가 $$a'$$를 argmax Q (=policy)에 따라 고르기 때문에 이는 아래와 같이 바꿔서도 표현 가능합니다.
...
&
max_{a_{i'}} Q_{\phi} (s'_i,argmax_{a'} Q_{\phi}) 
& \\
\end{aligned}
$$
