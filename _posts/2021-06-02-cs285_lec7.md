...
먼저 우리는 Supervised Learning에 대해 생각해봐야 되는데요, 이는 우리가 Value Function을 뉴럴 네트워크로 모델링 했기 때문에 현재 네트워크 구조에서 가능한 파라메터의 경우의 수를 모두 포함하는 continuous Value Function set을 가지고 있으며 Supervised Learning을 통해서 네트워크를 학습해 최적해를 찾는 것은 이 set중 최적의 값을 찾는다는 겁 됩니다.
...
여기서 Fitted Value Itearion에는 새로운 Operator $$\Pi$$가 추가된 것을 볼 수 있는데요, 이는 Projection Operator로 l2 norm 에 대해서 $$\Omega$$라는 곳으로 어떤 벡터를 사영시키는 operator입니다. 여기서 $$\Omega$$는 뉴럴네트워크 입니다.

![omega](/assets/images/CS285/lec-7/omega.png)
*Fig.*
