...
&
V(s_t) \leftarrow V(s_t) + \alpha ( \underbrace{r_t + \gamma V(s_{t+1})}{ \text{ TD Target } } - V(s_t))
...
& \scriptstyle{Temporal-Difference form 2}  \\

&
\text{ wait, how does it look like? }  \phi \leftarrow \phi - \alpha \frac{dL}{d \phi} \text{ gradient descent ! }
& \\ 
