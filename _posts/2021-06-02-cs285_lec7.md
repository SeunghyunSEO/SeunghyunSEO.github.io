...
&
\text{where } \prod \text{ is } \prod V = arg min_{V' \in \Omega} \frac{1}{2} \sum \parallel  V'(s) - V(s) \parallel^2
& \\
\end{aligned}
$$

여기서 Fitted Value Itearion에는 새로운 Operator $$\prod$$가 추가된 것을 볼 수 있는데요, 이는 Projection Operator로 l2 norm 에 대해서 $$\Omega$$라는 곳으로 어떤 벡터를 사영시키는 operator입니다. 여기서 $$\Omega$$는 뉴럴네트워크 입니다.

![omega](/assets/images/CS285/lec-7/omega.png)
*Fig.*

그러니까 Fitted Value Iteartion은
