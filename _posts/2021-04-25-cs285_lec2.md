...
왼쪽의 경우 카메라가 전면 유리 안쪽에 위치해 있어 대쉬보드 같은것들을 전부 관측 (observe)할 수 있는 상황인데, 브레이크를 밟으면 대쉬보드에 어떤 불빛이 꺼집니다.
처음 사람을 보고 브레이크를 밟겠지만 그 뒤로는 불빛이 꺼진 게 무엇때문이지 모델이 헷갈린다는 겁니다. (??? 별로 와닿지 않네요...)
...
매 순간마다 Softmax 확률이 주어질테고 그대로 행동하면 되니까요.

![additive_fig_12](/assets/images/CS285/lec-2/additive_fig_12.png){: width="50%"}
*Additive Fig.*

하지만 `Continuous`한 상황이라면 어떨까요? 우리는 `Gaussian Distribution`을 모델링 한다고 생각해 볼 수 있고, 이럴 경우 `평균 (mean)` 과 `분산 (varioance)`이 출력으로 나오는데 이를 어떻게 사용해서 결정을 내려야 할지 감이 잘 오지 않습니다. 하지만 대부분 사람이 결정을 내릴 때는 간단한 일을 할 때라도 매우 복잡한 분포 (very complicate distributions)을 이용해서 결정을 내린다고 하는데요, 이런게 강화학습에서는 문제가 된다는 것입니다.

![slide21](/assets/images/CS285/lec-2/slide21.png)
*Slide. 21.*

간단한 해결책은 바로 하나의 `평균 (mean)` 과 `분산 (varioance)`만 가지는 `One Mode Distribition` 을 사용하지 않고, 봉우리(Mode)가 여러개인 `Multi Modal Distribution`을 모델링 한다는 겁니다. 이럴 경우 Latent Variable Model을 사용할 수 있습니다. 혹은 `Autoregressive Discretization`을 사용 할 수도 있다고 하는데요, 이에 대해 조금 더 얘기 해 보도록 하겠습니다.

![slide22](/assets/images/CS285/lec-2/slide22.png)
*Slide. 22.*

가우시안 여러개를 weighted sum 하는 방법은 `Mixture Density Network`라고 불리기도 합니다. 여러개의 `평균 (mean)` 과 `분산 (varioance)`을 예측하고 이를 weighted sum 하는 거죠. 이러한 방법의 단점은 파라메터 수가 엄청나게 증가하며, 고차원의 분포를 모델링 하게 될 수록 학습이 어려워 진다는 겁니다. 즉 이미지를 이용해서 자율주행을 하는 task에서는 그다지 어렵지 않겠지만, 엄청나게 고차원 (고관절)을 다루는 Humanoid Robot을 다루는 task에서는 그렇지 않다는 겁니다.

![slide23](/assets/images/CS285/lec-2/slide23.png)
*Slide. 23.*

잠재 변수 모델 (Latent Variable Model)은 원래 모델에서 잠재 변수를 Integrated Out 해서 학습하는 모델인데요, *Slide. 23.*에서 잘 보시면 어떤 분포 (prior에서 샘플링한 벡터를 이미지에 추가하게 되는데, 즉 이는 이미지와 어떤 Noise를 더해서 가우시안 분포로 Mapping 하겠다는 얘기입니다. 그니까 유사한 이미지더라도 Noise에 따라 다른 분포로 Mapping이 될텐데 이를 어떻게 학습하는지에 대해서는 나중에 (Lec 18) 다루도록 한다고 합니다.  
