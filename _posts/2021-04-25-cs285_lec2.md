...
이는 굉장히 좋지 않은 bound라고 하는데요, 왜냐하면 Trajectory의 길이가 늘어날수록 Error가 제곱해서 (quadratically) 증가하기 때문입니다.
...
이번에는 좀 더 일반적인 분석을 해 보도록 하겠습니다.


이번에는 우리의 Policy가 Training Points 이외에도 같은 분포의 다른 Points 에서도 잘 작동한다고 생각해 보겠습니다.

$$
assume : \mathbb{E}_{p_{train}(s)}[ \pi_{\theta}(a  \neq \pi^{\ast}(s) \vert s)] \leq \epsilon 
$$
