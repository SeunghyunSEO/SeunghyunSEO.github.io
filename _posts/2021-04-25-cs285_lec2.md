...
이런식으로 학습을 하면 앞서 말한 `Distributional Drift Problem`이 일어나지 않을 거라는 겁니다.
...
이번에는 training loop 중간에 사람이 개입해야 하는 DAgger와 다른 방식으로 `Distributional Drift Problem`을 해결 해 보도록 하겠습니다.
...
위의 수식은 우리가 얼마나 실수했는지에 대해서 점수를 내겠군요.

우리가 분석 해 보고 싶은것은 위의 수식의 Upper Bound인데요, 이를 외줄타기 처럼 생각해서 한번 줄에서 떨어지면 그 뒤는 없는걸로 생각해서 (즉 한번 경로를 이탈하면 그 뒤로는 어떻게 할 줄 몰라서 뒤에도 다 실수했다고 counting 하는 것) 계산을 해 보는 겁니다.
