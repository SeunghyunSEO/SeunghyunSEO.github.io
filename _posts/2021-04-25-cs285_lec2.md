...
이러한 방법의 단점은 파라메터 수가 엄청나게 증가하며, 고차원의 분포를 모델링 하게 될 수록 학습이 어려워 진다는 겁니다. 
...
잠재 변수 모델 (Latent Variable Model)은 원래 모델에서 잠재 변수를 Integrated Out 해서 학습하는 모델인데요, *Slide. 23.*에서 잘 보시면 어떤 분포 (prior에서 샘플링한 벡터를 이미지에 추가하게 되는데, 즉 이는 이미지와 어떤 Noise를 더해서 가우시안 분포로 Mapping 해주는 모델입니다. 
그니까 유사한 이미지더라도 Noise에 따라 다른 분포로 Mapping이 된다는 거죠. 하지만 이는 학습하기가 굉장히 까다롭고 여러가지 trick들이 필요합니다. 
