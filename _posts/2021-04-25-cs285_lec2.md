...
다시 위에서 다뤘던 호랑이에 쫓기는 상황을 생각해볼까요? 우리는 데이터도 풍부하고 다른 이슈는 없다고 생각할 때, 우리가 원하는 것은 단지 호랑이한테 먹히지 않고 살아남는 것이 될겁니다. 
...
즉 호랑이한테 먹히는 경우에 대한 `기대 값 (Expectation)`을 최소화 하는게 목적인거죠.
