...
- Lec 9 : 다시 정책 경사 방법론으로 돌아와서, 이를 더 잘하기 위해서 어떤 트릭들을 쓸 수 있을까? `Adavanced Policy Gradient` -> TRPO, NPG, PPO (정책 경사 알고리즘이 너무 이상한 방향으로, 너무 크게 정책을 업데이트 하지 않고 잘 작동하도록 하는 트릭들을 수학적으로 증명(?)함)
